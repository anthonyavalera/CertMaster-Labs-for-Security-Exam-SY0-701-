...
Connect to the PC10 VM. Send Ctrl+Alt+Delete and, if needed, sign in as Jaime using Pa$$w0rd as the password.

Jaime is a member of the Domain Admins group. So, this user account is an administrator on the PC10 system.

Test ICMP connectivity between PC10 and DC10 by using Command Prompt and the ping DC10 command.
...
Run the following command to ping: ping DC10
...
Connect to the DC10 VM. Send Ctrl+Alt+Delete and sign in as Structureality\Administrator using Pa$$w0rd as the password.

Minimize or close Server Manager if it appears. It will not be used in this lab.

Test ICMP connectivity between DC10 and PC10 by using Command Prompt and the ping PC10 command.

Expand this hint for guidance.
Select Type here to search from the taskbar, type cmd, then select Command Prompt from the results.

Run the following command to ping: ping PC10
...
Set the inbound rules of File and Printer Sharing (Echo Request – ICMPv4-In) and File and Printer Sharing (Echo Request – ICMPv6-In) to block ICMP Echo Requests from all network profiles by using Windows Defender Firewall with Advanced Security.

Expand this hint for guidance.
Select Type here to search from the taskbar, type firewall, then select Windows Defender Firewall from the results.

In the Windows Defender Firewall window, select Advanced settings.

In the Windows Defender Firewall with Advanced Security window, select Inbound Rules.

In the list of Inbound Rules, scroll down to locate the rule that begins with File and Printer Sharing (Echo Request – ICMPv4-In).
...
Right-click the File and Printer Sharing (Echo Request – ICMPv4-In) rule and then select Properties.

On the File and Printer Sharing (Echo Request – ICMPv4-In) Properties window, select Block the connection, then select OK.

Right-click the File and Printer Sharing (Echo Request – ICMPv6-In) rule and then select Properties.

On the File and Printer Sharing (Echo Request – ICMPv6-In) Pproperties window, select Block the connection, then select OK.

Close Windows Defender Firewall with Advanced Security and the Windows Defender Firewall window.
...
Switch to the PC10 VM. Send Ctrl+Alt+Delete and, if needed, sign in as Jaime using Pa$$w0rd as the password.
...
What aspect of firewall rule manipulation within Windows Defender Firewall is most effective at stopping unwanted communications?

Defining a deny rule
Disabling an allow rule
Renaming a rule
Excluding a rule from a profile
...
Connect to the PC10 VM. If needed, send Ctrl+Alt+Delete and sign in as Jaime using Pa$$w0rd as the password.
...
Right-click LABFILES, and then select Properties.

On the LABFILES Properties window, select the Sharing tab, and then select Advanced Sharing.

On the Advanced Sharing window, select the Share this folder checkbox, and then select OK.

On the LABFILES Properties window, select Close.

Connect to the DC10 VM. If needed, send Ctrl+Alt+Delete and sign in as Structureality\Administrator using Pa$$w0rd as the password.

Attempt to access the LABFILES share from PC10.
...
In the address field of File Explorer, enter \\PC10.

The LABFILES share should be displayed.

Double-click the labfiles share.
What is the name of the first folder (alphabetically) in the LABFILES share?
...
Close any window open to the LABFILES share.

Connect to the PC10 VM. If needed, send Ctrl+Alt+Delete and sign in as Jaime using Pa$$w0rd as the password.

Disable File and Printer Sharing by using Windows Defender Firewall.

Expand this hint for guidance.
Select Type here to search from the taskbar, enter Firewall, and then select Windows Defender Firewall.

In the Windows Defender Firewall winddow, select Advanced settings.

In the Windows Defender Firewall with Advanced Security window, select Inbound Rules.

In the list of Inbound Rules, scroll down to locate the rule that begins with File and Printer Sharing (SMB-In) and has Private in the Profile column.

Right-click the File and Printer Sharing (SMB-In) | Private rule and then select Properties.

On the File and Printer Sharing (SMB-In) Properties window, select Enabled, select Block the connection, then select OK.

Right-click the File and Printer Sharing (SMB-In) | Domain rule and then select Properties.
...
Wait for a Network Error to be displayed. This error confirms that the firewall block of File and Printer Sharing is enforced on the client.

Select Cancel to close the Network Error window.
