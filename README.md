...
In your local browser's address bar, enter http://dftt.sourceforge.net/.
...
Select EXT3FS Keyword Search Test #1.
...
Look over the contents of this page. You may need to return to this page later in this exercise.
...
An icon of a DVD labeled as "Student-Resources-L06.ISO" should be on the Kali desktop, but it will be greyed out. Right-click on this DVD icon and select Mount Volume.
...
Enter cd /root/Downloads to change into the directory.

Enter ls -l to view the contents and verify that the 4-kwsrch-ext3.zip file is present and is the size of 3954200 bytes.

Enter unzip 4-kwsrch-ext3.zip to extract the contents of the zip archive into its default subdirectories.

Enter ls -l 4-kwsrch-ext3 to view the contents of the new subdirectories.
...
Return to your local browser's tab focused on dftt.sourceforge.net where the EXT3FS Keyword Search #1 page is displayed.

Locate the MD5 hash in the paragraph below the Downloads heading.
...
Enter: cat 4-kwsrch-ext3-hash.txt.
...
Create a SHA1 hash value from the file by entering: sha1sum nc.exe.
...
In your local browser's address bar, enter metadefender.opswat.com
...
Type or copy-n-paste the following hash into the Trust no field, then select Process.
...
...less
Perform additional searches for other files. You can use file names or hashes from your own system, or you can return to the Kali VM and obtain file names and hashes from there. For example, you could search for information on the klogger.exe file using the SHA1 hash of: 196BF6F43F85F97CC2851C840DA8E451256995CB.
...
...less
Since hashes produced by the yescrypt algorithm are extremely difficult to crack, generate your own password hashes to crack. Enter:
...
Generate a salted password hash by entering:
...
Use John the Ripper to perform a brute force password crack against the salted password hash by entering:
