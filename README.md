...
Connect to PC10, send Ctrl+Alt+Delete, and sign in as Jaime using Pa$$w0rd as the password.

In the Windows search bar, enter settings: system, and then select System.

Do not select System Information.

In the System window navigation pane, select Remote settings.

In the System Properties window, verify that the Remote tab is selected.

On the Remote tab, under the Remote Desktop heading, select Allow remote connections to this computer, and then select Select Users.
...
In the Select Users or Groups window, in Enter the object names to select, enter Rene, and then select OK.

In the Remote Desktop Users window, verify that Rene is now listed, then select OK.

In the System Properties window, select OK.
...
Switch to DC10, send Ctrl+Alt+Delete, and then sign in as Structureality\Administrator using Pa$$w0rd as the password.

Minimize or close Server Manager if it appears. It will not be used in this exercise.

On the Windows task bar, open Windows search, enter remote desktop, and then select Remote Desktop Connection.

In the Remote Desktop Connection window, enter PC10 in the Computer: field, and then select Connect.

On the Windows Security window, select More choices, then select Use a different account.

Enter Rene in the newly available User name field, then enter Pa$$w0rd in the Password filed, and then select OK.
...
Notice that your entire display or screen is filled with the Desktop of the remote system. On the Remote Desktop toolbar (located at the top of the screen), select the Minimize icon (which is a single undescore character (i.e., _)).
...
Return to the Remote Desktop window by selecting it from the Taskbar.

In the Windows search bar, enter settings: system, and then select System

Do not select System Information.

In the System window, in View basic information about your computer, in Computer name, domain, and workgroup settings section, verify that the Computer name is PC10.
...
Enter ipconfig to display the network configiuration of the PC10 system

What is the IP address of the PC10 system?

On the Remote Desktop toolbar, select Close (i.e. the X icon) to disconnect the connection.

On the Remote Desktop Connection pop-up window stating Your remote session will be disconnected select OK.

You are returned to the desktop of the DC10 virtual machine.
...
Switch to KALI and sign in as root using Pa$$w0rd as the password.

Open a Terminal Emulator window by using the icon menu.

The icon looks like a small computer screen showing a black background with a white dollar sign cursor prompt.

In the Terminal Emulator window, enter apt list openssh-server command to verify that openssh-Server is installed.

Verify that the output of the command displays the following statement in brackets: [installed, automatic], which indicates that the SSH service is already installed.
...
Determine whether password authentication is enabled in the /etc/ssh/sshd_config SSH configuration file. Enter the following command:
...
Confirm that the SSH server is already running on Kali by entering the following command:
...
Type q to exit the status display.

Determine the IPv4 address of the Kali Linux system’s eth0 (i.e., Ethernet Zero [i.e., the first NIC]) interface by entering ip a s eth0.

What is the IPv4 address (i.e., inet) of the eth0 interface of Kali?
...
Switch to PC10, send Ctrl+Alt+Delete, and then sign in as Jaime using Pa$$w0rd as the password.
...
On the PuTTY Configuration window, type 10.1.16.66 in the Host Name (or IP address) field.

Verify that 22 is present in the Port field. All other settings should remain at their default.

Select Open to initiate the SSH connection to the Kali VM.

On the PuTTY Security Alert pop-up window, select Accept.

In the 10.1.16.66 - PuTTY window, at the login as prompt, enter root.

In the 10.1.16.66 - PuTTY window, at the root@10.1.16.66's password: prompt, enter Pa$$w0rd.

You should be presented with the default SSH welcome message.

Enter hostname to view the name of the system

The output should be "kali".

Enter mkdir remote to create a folder on Kali in the current working directory.
...
In the Command Prompt window, enter ssh root@10.1.16.66.

If prompted to continue connecting, enter yes.

At the root@10.1.16.66's password: prompt, enter Pa$$w0rd.

You should be presented with the default SSH welcome message.

Disable the automatic connection message by entering the following command: touch ~/.hushlogin.
