...
Burp Suite needs to be configured to perform Proxy without intercept. Start by selecting the Kali Applications menu from the top taskbar. It is to the far left and is a stylized white dragon on a blue square.

Select the search field (it will be empty and be indicated by a white magnifying glass), type burp, then select burpsuite.

After about 10 seconds, a Burp Suite Community Edition pop-up window is displayed. It contains Terms and Conditions, select I Accept.

Another Burp Suite Community Edition pop-up window is displayed which welcomes you to the Burp Suite Community Edition. Select Temporary project, then select Next.
...
The main Burp Suite window is displayed. The default Learn, explore, and discover page is shown.
...
Enter vim /var/www/html/newproxy.bat
...
Select the MS10 VM. Send Ctrl+Alt+Delete and sign in as Jaime using Pa$$w0rd as the password.
...
In the Firefox address bar, enter http://10.1.16.66/newproxy.bat

This task simulates the victim clicking on the link in the phishing message.

The Firefox Downloads window should open and display the download progress.

Select newproxy.bat from the Downloads window.
...
An Open File - Security Warning pop-up window appears. Select Run.
...
Now you want to claim your free juice! Start Firefox again, and then in the address bar, enter juiceshop.local.
...
Since the email claimed you have a pre-established account using your company credentials, type jaime@structureality.com in the Email field and type Pa$$w0rd in the Password field, then select Log in.
...
Switch back to the KALI virtual machine and, if needed, sign in as root using Pa$$w0rd as the password.

The Burp Suite interface should still be open to the HTTP history sub-tab.

You need to find the last communication from the victim to the Juice Shop host of http://juiceshop.local. Select the MIME type column to sort by that value. If the column looks empty, then select it again. You want the sorting to be inverted so the MIME-type "text" communications are at the top.
...
The contents of the selected message will be shown in the bottom left pane of Burp Suite. You should see the following at the bottom of the communication:
