...
In your local browser's address bar, enter joesandbox.com.
...
On the JoeSandbox Cloud Basic pop-up, select the "x" in the top right corner to close it.

Look over the "Deep Malware Analysis" page of the Joe Sandbox Cloud service.
...
Select Results at the top of the page.
...
Look over the various recent result summaries.
...
In the Search (hash, ID, tag) field at the top of the page, enter Zeus.x86.
...
Select the HTML icon on the far left of the result with a "Time & Date" of 2022-04-21 21:51:18 +02:00.
...
Look over the wealth of information provided by the Joe Sandbox Cloud service regarding this malware.

From the "Mitre Att&ck Matrix" section of the Zeus.x86 analysis report, what attack technique groups (i.e., columns) does this malware demonstrate? (Select all that apply)

Lateral Movement
Credential Access
Command and Control
Privilege Escalation
After you have looked over the lengthy report, close the tab displaying the "Linux Analysis Report: Zeus.x86" page.

In the Search (hash, ID, tag) field at the top of the page, enter Mirai.
...
Look over the results list for Mirai malware. Notice that the percent of detection by Antivirus varies amongst the tested samples.

Select the HTML icon to view the report of any of the Mirai results.

A detailed report of the tested sample related to Mirai will be displayed in a new tab.

Look over the wealth of information provided by the Joe Sandbox Cloud service regarding this malware.

After you have looked over the lengthy report, close the tab displaying the report.

Select Results at the top of the page.

This clears the search results and returns you to the full list of recently submitted samples.

Notice along the top of the page are malware terms on red buttons with a fire icon. These are malware concepts that have a recent high occurrence rate. Select one of these threat name buttons to investigate a concept further.
...
In your local browser's address bar, enter eicar.org.
...
Select the DOWNLOAD ANTI MALWARE TESTFILE graphic on the home page.

Clicking this link DOES NOT download anything to your local machine.

Scroll down to view the download links located in the middle of the page. You should see the ability to download four files: eicar.com, eicar.com.txt, eicar_com.zip, and eicarcom2.zip.
...
Leave your local browser's tab open to this EICAR DOWNLOAD ANTI MALWARE TESTFILE page.
...
An icon of a DVD labeled as "Student-Resources-L27.ISO" is on the Kali desktop, but it will be greyed out. Right-click on this DVD icon and select Mount Volume.
...
Enter cd /root/Downloads, then ls -l.

You should see both eicarcom2.zip and eicar.com.txt.

Type cat eicar.com.txt to view the contents of the file. It should be:
...
...less
Enter the following commands to extract the contents of the EICAR zip and view the contents:
...
Return your local browser's tab open to the EICAR DOWNLOAD ANTI MALWARE TESTFILE page.

Right-click the TXT: eicar.com download link and select Copy link address.
...
Open a new tab in your local browser. Leave the tab focused on eicar.org open.

In the address bar of the new tab of your local browser, enter virustotal.com.
...
...less
Select URL to select the URL tab of the home page of VirusTotal.
...
Select the *Search or scan a URL field, then paste (using CTRL-V or equivalent) the eicar.org URL from your clipboard, then press Enter on your keyboard.
...
...less
Look over the results of the scan on the DETECTION tab.

Notice that while some security products detected the EICAR test string, some do not.

Select the DETAILS tab, then look over the provided information.

When was this EICAR URL first seen in the wild?
...
...less
Select each of the other tabs available and look over the provided information.
...
Select Reanalyze (which is a circled arrow) near the top-right of the page.
...
In the Terminal window, enter sha256sum eicarcom2.zip to generate a hash of that file. The result should be:

e1105070ba828007508566e28a2b8d4c65d192e9eaf3b7868382b7cae747b397
Double-click the SHA256 hash in the previous instructions step, then right-click and select Copy.
...
Return your local browser's tab open to the VirusTotal page.

Select Return to Front Page (which looks like a blue bookmark or flag) at the top-left corner of the page.

You should be back at the initial page of VirusTotal.

Select the SEARCH tab.

Select the URL, IP address, domain, or file hash text field, right-click, select Paste.

The SHA256 hash of the eicarcom2.zip file should now be present on the SEARCH tab. Press Enter on your keyboard.
...
...less
The results of the hash database search will be displayed.

Notice that VirusTotal was able to identify the file as eicarcom2.zip.

Look over the analysis report. Be sure to look at the information on each of the tabs.

On what tab of the VirusTotal analysis report will you find information about execution parents and bundled files?
