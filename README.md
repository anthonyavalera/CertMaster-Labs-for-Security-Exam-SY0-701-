...
cipher /r:EFSRA
At the prompt to provide a password to protect the .PFX file, enter Pa$$w0rd.

Confirm the password Pa$$w0rd.
...
On the PC10 virtual machine, send Ctrl+Alt+Delete and sign in as Admin with the password Pa$$w0rd.
...
Connect to the PC10 virtual machine, and if needed, send Ctrl+Alt+Delete, and then sign in as Admin with the password Pa$$w0rd.
...
Why is the Pat account no longer able to open their encrypted files?

Encrypted files can only be opened during working hours.
An administrator forcibly changed their password.
The files are owned by another accounts.
They are signed into the wrong computer.
Sign out Pat from PC10.
...
Enter Pa$$w0rd in the Password: field, leave all other settings at their defaults, then select Next.
