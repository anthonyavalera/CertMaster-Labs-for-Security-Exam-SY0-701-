...
Enter fdisk <SDB> to initate the tool to create storage partitions.
...
Enter p print (i.e., display) the current partition table.
...
Enter mkfs -t fat <SDB1> to format the new partition with the FAT32 file format.
...
Enter mount <SDB1> /mnt/SalesStorage to mount the newly formatted partition to the mount point location.
...
A listing of the contents of the root account's home folder will be displayed. You need to select an output destination. Use your keyboard's down arrow key to highlight Downloads, press Enter to open the Downloads directory, then type a capital C on your keyboard to set the destination directory.
...
Since the data on the drive was formatted in the previous exercise, use the following commands to place data on the drive again:
